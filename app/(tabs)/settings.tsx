          ...
          <View className="rounded-2xl border border-stone dark:border-[#2E2E2C] bg-white dark:bg-[#1E1E1C] overflow-hidden mb-7">
            {THEME_OPTIONS.map((opt, i) => {
              ...
              return (
                ...
                >
                  <View className={`w-9 h-9 rounded-xl items-center justify-center ${selected ? 'bg-mist dark:bg-[#0D2B1A]' : 'bg-stone dark:bg-[#2E2E2C]'}`}>
                    <Ionicons name={opt.icon} size={18} color={selected ? iconPrimary : iconMuted} />
                  </View>
                  <View className="flex-1">
                    <Text className="text-[15px] font-medium text-ink dark:text-[#F2F0EB]">{opt.label}</Text>
          ...
          <View className="rounded-2xl border border-stone dark:border-[#2E2E2C] bg-white dark:bg-[#1E1E1C] overflow-hidden mb-7">
            {[
              ...
              >
                ...
                <Text className="flex-1 text-[15px] font-medium text-ink dark:text-[#F2F0EB]">{row.label}</Text>
            ...
            {MEMBERS.map((name, i) => (
              ...
              >
                <View className="w-8 h-8 rounded-full bg-mist dark:bg-[#0D2B1A] items-center justify-center">
                  <Text className="text-[13px] font-bold text-forest dark:text-mint">{name.charAt(0)}</Text>
                </View>
